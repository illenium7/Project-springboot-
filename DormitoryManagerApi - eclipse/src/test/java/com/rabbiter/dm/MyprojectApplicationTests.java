...
import com.rabbiter.dm.dao.*;
import com.rabbiter.dm.entity.*;
...
//        System.out.println(list);
    ...
    void testRoom() {
        for (int j = 1; j < 6; j++) {
            for(int i = 1;i < 11; i++) {
                ...
                Room room = new Room();
                room.setMaxCapacity(6);
                room.setBuildingId(18L);
    ...
    void log() {
        ...
        systemLog.setIp("172");
        systemLog.setClas("user");
        systemLog.setMethod("list");
        systemLog.setUserId(2L);
        systemLog.setOperateTimeStart(LocalDateTime.now());
        systemLog.setOperateTimeEnd(LocalDateTime.now());
        List<SystemLog> logs = systemLogMapper.list(systemLog);
